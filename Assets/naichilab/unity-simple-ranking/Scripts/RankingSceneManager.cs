...
namespace naichilab
{
    public class RankingSceneManager : MonoBehaviour
    {
        ...
        private const string COLUMN_ICON = "gameID"; 
        ...
        void Start()
        {
            ...
            _gameID = RankingLoader.Instance.gameID;
        ...
        private IEnumerator SendScoreEnumerator()
        {
            ...
            _ncmbRecord[COLUMN_ICON] = _gameID;
        ...
        private IEnumerator LoadRankingBoard()
        {
            ...
            else if (so.Count > 0)
            {
                ...
                foreach (var r in so.Result)
                {
                    ...
                    char[] chars = r[COLUMN_ICON].ToString().ToCharArray();
                    string[] iconID = Array.ConvertAll(chars, c => c.ToString());
                    rankNode.gameID.sprite  = icons[int.Parse(iconID[0])]; 
                    rankNode.gameID1.sprite = icons[int.Parse(iconID[1])];
                    rankNode.gameID2.sprite = icons[int.Parse(iconID[2])];
                    rankNode.gameID3.sprite = icons[int.Parse(iconID[3])];
                    rankNode.gameID4.sprite = icons[int.Parse(iconID[4])];
